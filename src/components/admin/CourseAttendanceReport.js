...
import {View, Text, StyleSheet, TouchableHighlight, ScrollView, ToastAndroid} from 'react-native'
import Icon from 'react-native-vector-icons/FontAwesome'
import Axios from 'axios'

import config from '../../config'

export default class CourseAttendanceReport extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: []
  ...
  buildUrlQuery = (courses) => {
    let url = config.baseUrl + '/attendance/find_by_courses/?';
    for (let i = 0; i < courses.length; i++) {
      url = url.concat('course_code=' + escape(courses[i]) + '&')
    }
  ...
  componentDidMount() {
    const course_codes = this.props.navigation.state.params['courses']
    let url = this.buildUrlQuery(course_codes)
    Axios.get(url)
    .then(response => {
      ...
      let data = this.arrangeByCourses(byCourses, this.getStudents(byCourses))
      this.setState({data})
    })
      ...
      <View style={{flex: 1}}>
        ...
        <ScrollView style={{flex: 1}}>
          <View style={styles.contentContainer}>
            ...
            {
              this.state.data.map(atd => <EachCourse key={atd.course} data={atd} />)
            }
          </View>
        </ScrollView>
      </View>
    ...
    students.forEach(student => {
      byCourses.forEach(({course, _absent, _present, _excused, totalAttendance}) => {
        ...
        if (mStudents.has(student)) {
          ...
          mStudents.set(student, [...current, data])
        } else {
          mStudents.set(student, [data])
        }
      })
    })
...
class EachCourse extends Component {
  ...
  render() {
    let {course, data, totalAttendance} = this.props.data;
    ...
    return (
      <View style={styles.card}>
        ...
        <View style={styles.list}>
          ...
          {
            data.map(({username, _absent, _excused, _present}) => {
              return(
                <View key={username} style={styles.listItem}>
                  <Text style={[styles.listItemText, styles.listFirstItem]}>{username}</Text>
